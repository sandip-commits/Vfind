...
function parseExperienceStringToYears(exp?: string): number {
  ...
  if (n.includes("less than 6") || n.includes("less than 6 months")) return 0.25;
...
export default function JobData() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        // ensure we at least get an array
  ...
  // parse job's experience into a single numeric value (years)
  const getJobExperienceYears = (job: Job) => {
    // prefer explicit numeric experienceMin
    ...
    // fallback: try parseExperienceStringToYears on experienceMin or description
    return (
  ...
  // apply filters (debounced by useEffect below)
  const applyFilters = useCallback(() => {
    ...
    if (typeFilter.length > 0) {
      temp = temp.filter((job) => (job.type ? typeFilter.some((t) => normalizeStr(job.type).includes(normalizeStr(t))) : false));
    }

    if (shifts.length > 0) {
      temp = temp.filter((job) => job.shift && shifts.some((s) => normalizeStr(job.shift).includes(normalizeStr(s))));
    }
    ...
    // Pay Rate filter (use job.maxPay or minPay if available)
    if (payRate > 0) {
      temp = temp.filter((job) => {
  ...
  }, [search, location, typeFilter, shifts, roleCategories,  experience, payRate, radius, applyFilters]);
        ...
        <div className="flex-1 max-w-[983px]">
          <div className="grid grid-cols-1 gap-4">
            {filteredJobs.map((job) => (
              <div key={job.id} className="flex justify-between items-center bg-white rounded-lg p-6 shadow-sm hover:shadow-md transition-all duration-300 mx-2">
                {/* Left */}
                <div className="flex-1">
                  <h2 className="font-semibold text-bold text-lg text-[#61A6FA] mb-1">{job.title}</h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-gray-600 text-sm">
                    ...
                    <div className="flex items-center gap-1">
                      <DollarSign size={16} />
                      <span>
                        {job.minPay || job.maxPay ? `$${job.minPay || "0"} - $${job.maxPay || "0"}/hr` : "Not specified"}
                      </span>
                    </div>
                ...
                <div className="flex items-center gap-4 ml-6 flex-col">
                  <Link href={`/nurseProfile/jobapplicationpage/${job.id}`} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-[#61A6FA] text-white text-sm font-medium rounded-[10px] hover:bg-blue-500 transition-all duration-200">
                    View Details
                  </Link>
                </div>
              </div>
